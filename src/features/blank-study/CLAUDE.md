...
| 옵션 | 값 | 비고 |
|------|----|------|
| 출처 | `pdf` \| `explanation` | PDF 원문 vs 생성된 강의 노트 |
| 범위 | `all` \| `chapter` \| `page` | **PDF 출처일 때만** 표시 |
| 밀도 | `light` \| `normal` \| `dense` | mock item 수 |

**출처 규칙**:
- `pdf`: 저장된 PDF 추출 텍스트(`MaterialPdfText`)에서 원문 문장에 빈칸 — 수작업 seed 아님
- `explanation`: `LectureExplanation`(핵심 개념/시험 강조 등)에서 추출
- `explanation` 선택 + 설명 없음 → **생성 차단** + 쉽게 설명 탭 CTA (자동 폴백 금지)

## 결과 UI (`BlankStudyResultView`)
...
- 컴포넌트: `BlankGeneratePanel`, `BlankStudyResultView`, `BlankProseSection`, `BlankPdfProseView`, `BlankSelectableText`
- 훅: `hooks/useBlankStudy.ts` / 타입: `src/types/blank-study.ts`
- 로직: `src/lib/blank-study/` (build, tokenize, add/remove custom blank, normalize-answer)
- mock: `src/lib/mocks/blank-study.ts`, `blank-study-content.ts` / 라우트: `src/app/(app)/study/[id]/blank-study/page.tsx`
