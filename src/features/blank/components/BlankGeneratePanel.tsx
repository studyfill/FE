...
import { BlankResultView } from "@/features/blank/components/BlankResultView"
...
export const BlankGeneratePanel = ({
  ...
  const isNoteSource = options.source === "note"
  const needsNote = isNoteSource && !hasNote
  const extractionReady = userFile.extractionStatus === "done"
          ...
          <div className="flex w-full max-w-generate-panel -translate-y-4 flex-col gap-8">
            ...
            <div className="space-y-3">
              <Button
                ...
                disabled={
                  isGenerating || !extractionReady || needsNote
                }
                aria-label="빈칸 생성하기"
              >
                {isGenerating ? (
