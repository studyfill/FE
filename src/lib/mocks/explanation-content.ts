...
const isTreeMaterial = (name: string) =>
  /트리|tree|bst|이진/i.test(name)

const isOsMaterial = (name: string) =>
  /운영체제|os|프로세스|스케줄/i.test(name)

const toTopicTitle = (name: string) =>
  name.replace(/\.pdf$/i, "").replace(/^.*[\\/]/, "").trim()

export const buildMockExplanation = (
  material: Material,
  options: ExplanationGenerateOptions
): Omit<LectureExplanation, "materialId" | "generatedAt"> => {
  if (isTreeMaterial(material.name)) {
    return buildTreeExplanation(material, options)
  }

  if (isOsMaterial(material.name)) {
    return buildOsExplanation(material, options)
  }

  return buildGenericExplanation(material, options)
}

const buildTreeExplanation = (
  material: Material,
  options: ExplanationGenerateOptions
): Omit<LectureExplanation, "materialId" | "generatedAt"> => {
  const title = toTopicTitle(material.name).includes("트리")
    ? "트리와 이진탐색트리"
    : toTopicTitle(material.name)
...
const buildOsExplanation = (
  material: Material,
  options: ExplanationGenerateOptions
): Omit<LectureExplanation, "materialId" | "generatedAt"> => {
  const title = toTopicTitle(material.name)
